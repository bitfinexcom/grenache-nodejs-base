...
class Link extends Events {
  ...
  connect () {
    ...
    this._socket.on('close', this.disconnect.bind(this))
  }
  ...
  request (type, payload, opts, cb) {
    if (!this._connected) {
      return cb(new Error('ERR_GRAPE_READY'))
    }
  ...
  newRequest (type, payload, _opts, cb) {
    ...
    const opts = _.extend({
      timeout: this.conf.linkRequestTimeout
    }, _opts)
  ...
  monitor () {
    ...
    if (!this._pingPending) {
      ...
      this.request(
        'lookup', 'grenache:ping', { timeout: this.conf.pingTimeout },
        (err, res) => {
          this._pingPending = false
          if (err) {
            debug(`Link error ${this.conf.grape}: ${err}`)
            this.disconnect()
          }
        }
      )
